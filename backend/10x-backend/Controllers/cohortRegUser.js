const CohortRegUser = require("../Models/cohortRegUser");
const UnRegister = require("../Models/cohortUnregUser");
...
const signup = async (req, res) => {
    ...
    const user = await UnRegister.findById(registerationId);
    ...
    try {
        if (!user) {
            ...
            if (user.otp == otp) {
                const sUser = await CohortRegUser.create({
                    name: user.name,
                    mobile: user.mobile,
                ...
                await UnRegister.deleteOne({ _id: registerationId });
...
const loginInit = async (req, res) => {
    ...
    try {
        const user = await CohortRegUser.findOne({ mobile });
...
const resetPasswordInit = async (req, res) => {
    ...
    try {
        const user = await CohortRegUser.findOne({ $or: [{ gMobile }, { gEmail }] });
