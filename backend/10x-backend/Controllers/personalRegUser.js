...
const PUnRegistered = require("../Models/personalUnreg");  
...
const signup = async (req, res) => {
    ...
    const user = await PUnRegistered.findById(registerationId);
    ...
    try {
        if (!user) {
            ...
            if (user.otp == otp) {
                const sUser = await PUser.create({
                    name: user.name,
                    mobile: user.mobile,
                ...
                await PUnRegistered.deleteOne({ _id: registerationId });
...
const loginInit = async (req, res) => {
    ...
    try {
        const user = await PUser.findOne({ mobile });
...
const resetPasswordInit = async (req, res) => {
    ...
    try {
        const user = await PUser.findOne({ $or: [{ gMobile }, { gEmail }] });
